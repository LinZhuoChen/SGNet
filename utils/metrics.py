...
class IOUMetric:
    ...
    def add_batch(self, predictions, gts, ignore_index):
        ignore_index = gts != 255
        predictions = predictions[ignore_index]
        gts = gts[ignore_index]
        self.hist += self._fast_hist(predictions, gts)

    def add_batch_ignore(self, predictions, gts, ignore_index):
        for lp, lt in zip(predictions, gts):
            ignore_index = lt != 255
            lp = lp[ignore_index]
            lt = lt[ignore_index]
            self.hist += self._fast_hist(lp, lt)
