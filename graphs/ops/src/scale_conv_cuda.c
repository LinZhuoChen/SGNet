...
void shape_check(THCState *state, THCudaTensor *input, THCudaTensor *scale,
...
//  long nOutputPlane = weight->size[0];
  ...
  THArgCheck(
      (THCudaTensor_size(state, scale, 2) == outputHeight &&
      THCudaTensor_size(state, scale, 3) == outputWidth), 3,
      "invalid spatial size of scale, expected height: %d width: %d, but got height: %d width: %d",
      outputHeight, outputWidth, THCudaTensor_size(state, scale, 2),
      THCudaTensor_size(state, scale, 3));

  THArgCheck((THCudaTensor_size(state, scale, 1) == deformable_group * 2), 3,
             "invalid number of channels of scale");
...
int scale_conv_forward_cuda(THCudaTensor *input, THCudaTensor *weight,
  ...
  THCAssertSameGPU(THCudaTensor_checkGPU(state, 6, input, weight, scale,
                                         output, columns, ones));

  shape_check(state, input, scale, NULL, weight, kH, kW, dH, dW, padH, padW,
              dilationH, dilationW, deformable_group);

  input = THCudaTensor_newContiguous(state, input);
  scale = THCudaTensor_newContiguous(state, scale);
  ...
  if (THCudaTensor_nDimension(state, input) == 3) {
    ...
    THCudaTensor_resize4d(state, scale, 1, THCudaTensor_size(state, scale, 0), THCudaTensor_size(state, scale, 1),
                          THCudaTensor_size(state, scale, 2));
  }
  ...
  THArgCheck((THCudaTensor_size(state, scale, 0) == batchSize), 3, "invalid batch size of scale");
  ...
  THCudaTensor *scale_n = THCudaTensor_new(state);
  ...
  THCudaTensor_resize5d(state, scale, batchSize / im2col_step, im2col_step,
      deformable_group * 2, outputHeight, outputWidth);

  for (int elt = 0; elt < batchSize / im2col_step; elt++) {

    THCudaTensor_select(state, input_n, input, 0, elt);
    THCudaTensor_select(state, scale_n, scale, 0, elt);
    ...
    scale_im2col(
        ...
        THCudaTensor_data(state, scale_n), nInputPlane, inputHeight,
  ...
  THCudaTensor_resize4d(state, scale, batchSize, 2, outputHeight, outputWidth);

  THCudaTensor_free(state, input_n);
  THCudaTensor_free(state, scale_n);
  ...
  if (batch == 0) {
    ...
    THCudaTensor_resize3d(state, scale, THCudaTensor_size(state, scale, 1),
        THCudaTensor_size(state, scale, 2), THCudaTensor_size(state, scale, 3));
  }

  THCudaTensor_free(state, input);
  THCudaTensor_free(state, scale);
...
int scale_conv_backward_input_cuda(
  ...
  THCAssertSameGPU(THCudaTensor_checkGPU(state, 6, input, gradOutput, weight,
                                         scale, columns, gradInput));

  shape_check(state, input, scale, gradOutput, weight, kH, kW, dH, dW, padH,
  ...
  THCudaTensor *gradScale_n = THCudaTensor_new(state);
  THCudaTensor *input_n = THCudaTensor_new(state);
  THCudaTensor *scale_n = THCudaTensor_new(state);
  ...
  THCudaTensor_resize5d(state, input, batchSize / im2col_step, im2col_step, nInputPlane, inputHeight, inputWidth);
  THCudaTensor_resize5d(state, gradScale, batchSize / im2col_step, im2col_step,
      2 * deformable_group, outputHeight, outputWidth);
  THCudaTensor_resize5d(state, scale, batchSize / im2col_step, im2col_step,
      2 * deformable_group, outputHeight, outputWidth);
  ...
  for (int elt = 0; elt < batchSize / im2col_step; elt++) {
    ...
    THCudaTensor_select(state, gradScale_n, gradScale, 0, elt);
    THCudaTensor_select(state, input_n, input, 0, elt);
    THCudaTensor_select(state, scale_n, scale, 0, elt);
    ...
    scale_col2im_coord(
        ...
        THCudaTensor_data(state, input_n), THCudaTensor_data(state, scale_n),
        nInputPlane, inputHeight, inputWidth, kH, kW, padH, padW, dH, dW,
        dilationH, dilationW, im2col_step, deformable_group,
        THCudaTensor_data(state, gradScale_n));
  ...
  THCudaTensor_resize4d(state, gradInput, batchSize, nInputPlane, inputHeight, inputWidth);
  THCudaTensor_resize4d(state, input, batchSize, nInputPlane, inputHeight, inputWidth);
  THCudaTensor_resize4d(state, gradScale, batchSize, deformable_group * 2 * kH * kW, outputHeight, outputWidth);
  THCudaTensor_resize4d(state, scale, batchSize, 2, outputHeight, outputWidth);

  THCudaTensor_free(state, gradInput_n);
  THCudaTensor_free(state, gradScale_n);
  THCudaTensor_free(state, input_n);
  THCudaTensor_free(state, scale_n);
  ...
  if (batch == 0) {
    ...
    THCudaTensor_resize3d(state, gradInput, nInputPlane, inputHeight,
                          inputWidth);
    THCudaTensor_resize3d(state, scale, THCudaTensor_size(state, scale, 1), THCudaTensor_size(state, scale, 2),
                          THCudaTensor_size(state, scale, 3));
    THCudaTensor_resize3d(state, gradScale, THCudaTensor_size(state, scale, 1), THCudaTensor_size(state, scale, 2),
                          THCudaTensor_size(state, scale, 3));
  }
...
int scale_conv_backward_parameters_cuda(
  ...
  // todo: add im2col_step as input
  THCAssertSameGPU(THCudaTensor_checkGPU(state, 5, input, scale, gradOutput,
                                         gradWeight, columns));

  shape_check(state, input, scale, gradOutput, gradWeight, kH, kW, dH, dW,
  ...
  for (int elt = 0; elt < batchSize / im2col_step; elt++) {
    ...
    THCudaBlas_Sgemm(state, 't', 'n', n, m, k, scale_,
